...
import ProductView from "../../SingleServicePage/ProductView";
...
export default function ProductCardStyleOne({ datas }) {
  ...
  const quickViewHandler = (slug) => {
    ...
    if (!quickViewData) {
      axios
        .get(`${process.env.NEXT_PUBLIC_BASE_URL}api/service/${slug}`)
        .then((res) => {
  ...
  return (
    <>
    <Link
    href={{ pathname: "/single-service", query: { slug: datas.slug } }}
    passHref
  >
